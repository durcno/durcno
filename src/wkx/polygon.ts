...
export class Polygon extends Geometry {
  ...
  static _parseWkt(value: WktParser, options: WktParseOptions): Polygon {
    ...
    value.expectGroupStart();
    polygon.exteriorRing = polygon.exteriorRing.concat(
  ...
  static _parseWkb(value: BinaryReader, options: WkbOptions): Polygon {
    const polygon = new Polygon();
    polygon.srid = options.srid;
  ...
  static _parseTwkb(value: BinaryReader, options: TwkbOptions): Polygon {
    const polygon = new Polygon();
    polygon.hasZ = options.hasZ || false;
    polygon.hasM = options.hasM || false;
  ...
  static _parseGeoJSON(value: GeoJSON.Polygon): Polygon {
    const polygon = new Polygon();
  ...
  toGeoJSON(options?: GeoJSONOptions): GeoJSON.Polygon & { crs?: any } {
    const geoJSON = super.toGeoJSON(options) as GeoJSON.Polygon & { crs?: any };
    geoJSON.type = Types.geoJSON.Polygon as "Polygon";
    geoJSON.coordinates = [];

    if (this.exteriorRing.length > 0) {
      const exteriorRing: GeoJSON.Position[] = [];
    ...
    for (let j = 0; j < this.interiorRings.length; j++) {
      const interiorRing: GeoJSON.Position[] = [];
