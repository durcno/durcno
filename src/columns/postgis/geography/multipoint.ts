...
import { Sql } from "../../../sql";
import { MultiPoint } from "../../../wkx";
import { Column, type ColumnConfig } from "../../common";

type As = "xy" | "xyz" | "xym" | "xyzm";

export type MultiPointConfig = ColumnConfig & {
  srid?: number;
  format?: "xy";
};
...
> {
  ...
  fromDriverScalar(value: string | null): GetValType<TConfig["format"]> | null {
    ...
    const geom = MultiPoint.parse(buffer, {
      srid: this.#srid,
      hasZ: this.#hasZ,
      hasM: this.#hasM,
    }) as MultiPoint;
 ...
 */
export function multipoint<TConfig extends MultiPointConfig>(
  config: TConfig,
): MultiPointColumn<TConfig> {
