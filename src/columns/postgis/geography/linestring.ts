...
export type LineStringConfig = ColumnConfig & {
  srid?: number;
  format?: "xy";
};
...
export class LineStringColumn<TConfig extends LineStringConfig> extends Column<
  TConfig,
...
> {
  ...
  get zodTypeScaler(): GetZodType<TConfig["format"]> {
    ...
    return z.array(pointTuple) as GetZodType<TConfig["format"]>;
  }
  ...
  fromDriverScalar(value: string | null): GetValType<TConfig["format"]> | null {
    ...
    const geom = LineString.parse(buffer, {
      srid: this.#srid,
      hasZ: this.#hasZ,
      hasM: this.#hasM,
    }) as LineString;
 ...
 */
export function linestring<TConfig extends LineStringConfig>(
  config: TConfig,
): LineStringColumn<TConfig> {
