...
await db.insertInto(Networks).values({
...
await db.insertInto(NetworkDevices).values({
  name: "Router 1",
...
Marks the column as `GENERATED ALWAYS`. When used with `.as(identity)`, this creates a PostgreSQL identity column that the database always populates — you cannot insert or update the value manually. The column becomes non-insertable (`never` on insert).

```typescript
import { bigint, identity, primaryKey } from "durcno";

// GENERATED ALWAYS AS IDENTITY — database controls the value entirely
id: bigint({ primaryKey }).generatedAlways().as(identity),
```

#### `.generatedByDefault()`

Marks the column as `GENERATED BY DEFAULT`. When used with `.as(identity)`, this creates a PostgreSQL identity column that the database populates unless you supply a value explicitly. The column becomes optional on insert.

```typescript
import { integer, identity } from "durcno";

// GENERATED BY DEFAULT AS IDENTITY — you may still supply a value
counter: integer({}).generatedByDefault().as(identity),
```

#### `.as(expr)`

Sets the expression or keyword that follows `GENERATED (ALWAYS|BY DEFAULT) AS`. Pass the exported `identity` constant for PostgreSQL identity columns, or a raw SQL `string` / `Sql` expression for generated computed columns.

```typescript
import { integer, identity, sql } from "durcno";

// Identity column
id = integer({ primaryKey }).generatedAlways().as(identity),

// Computed column (expression)
fullName = text({}).generatedAlways().as(sql`first_name || ' ' || last_name`),
```

:::tip
Use `.generatedAlways().as(identity)` for surrogate primary keys that are entirely managed by PostgreSQL. Prefer `pk()` as a shorthand — it expands to `bigint({ primaryKey }).generatedAlways().as(identity)`.
:::
