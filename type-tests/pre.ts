...
const basicQuery = prepare({ username: Users.username.arg() }, (args) => {
  return db
    .prepare()
    .from(Users)
    .select("*")
...
const selectiveQuery = prepare({ id: Users.id.arg() }, (args) => {
  return db
    .prepare()
    .from(Users)
...
const numericQuery = prepare({ userId: Users.id.arg() }, (args) => {
  return db
    .prepare()
    .from(Posts)
    .select("*")
...
const enumQuery = prepare({ userType: Users.type.arg() }, (args) => {
  return db
    .prepare()
    .from(Users)
...
const nullableQuery = prepare({ email: Users.email.arg() }, (args) => {
  return db
    .prepare()
    .from(Users)
    .select("*")
...
const singleColumnQuery = prepare({ id: Users.id.arg() }, (args) => {
  return db
    .prepare()
    .from(Users)
...
const timestampQuery = prepare({ createdAt: Users.createdAt.arg() }, (args) => {
  return db
    .prepare()
    .from(Users)
...
const wrongArgTypeQuery = prepare({ id: Users.id.arg() }, (args) =>
  db
    .prepare()
    .from(Users)
    .select("*")
...
const enumArgQuery = prepare({ type: Users.type.arg() }, (args) =>
  db
    .prepare()
    .from(Users)
    .select("*")
    .where(() => eq(Users.type, args.type)),
);
// @ts-expect-error - Invalid enum value at runtime should not compile
enumArgQuery.run(db, { type: "invalid_type" });

const bigintArgQuery = prepare({ userId: Posts.userId.arg() }, (args) =>
  db
    .prepare()
    .from(Posts)
    .select("*")
...
const extraArgQuery = prepare({ id: Users.id.arg() }, (args) =>
  db
    .prepare()
    .from(Users)
    .select("*")
...
const updateBasicQuery = prepare({ userId: Users.id.arg() }, (args) => {
  return db
    .prepare()
    .update(Users)
...
const updateMultiArgQuery = prepare(
...
const updateReturningQuery = prepare({ userId: Users.id.arg() }, (args) => {
  return db
    .prepare()
    .update(Users)
...
const updatePostsQuery = prepare(
  { postId: Posts.id.arg(), userId: Posts.userId.arg() },
  (args) => {
    return db
      .prepare()
...
const wrongUpdateArgType = prepare({ id: Users.id.arg() }, (args) =>
  db
    .prepare()
    .update(Users)
    .set({ username: "test" })
...
const missingUpdateArg = prepare(
...
const deleteMultiArgQuery = prepare(
  { userId: Users.id.arg(), userType: Users.type.arg() },
  (args) => {
    return db
      .prepare()
...
const deletePostsPreQuery = prepare(
  { postId: Posts.id.arg(), userId: Posts.userId.arg() },
  (args) => {
    return db
      .prepare()
...
const missingDeleteArg = prepare(
  { id: Users.id.arg(), type: Users.type.arg() },
  (args) =>
    db
      .prepare()
...
const countPreparedQuery = prepare({ userType: Users.type.arg() }, (args) => {
...
const existsPreparedQuery = prepare(
  { username: Users.username.arg() },
  (args) => {
...
const firstPreparedQuery = prepare({ userId: Users.id.arg() }, (args) => {
...
const sumPreparedQuery = prepare({ userType: Users.type.arg() }, (args) => {
...
const avgPreparedQuery = prepare({ userType: Users.type.arg() }, (args) => {
...
const minPreparedQuery = prepare({ userType: Users.type.arg() }, (args) => {
...
const maxPreparedQuery = prepare({ userType: Users.type.arg() }, (args) => {
...
const distinctPreparedQuery = prepare(
